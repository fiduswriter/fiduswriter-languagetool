...
export class EditorLT {
    ...
    init() {
        ...
        this.editor.statePlugins.push([
            languagetoolPlugin,
            () => ({editor: this.editor, editorLt: this})
        ])
    ...
    initSources() {
        this.sources = []
        this.editor.view.state.doc.firstChild.forEach(
            (_node, _offset, index) => {
                const language =
                    this.editor.view.state.doc.firstChild.child(index).attrs
                        .language ||
                ...
                this.sources.push({
                    ...
                    getNodes: () => [
                        this.editor.view.state.doc.firstChild.child(index)
                    ],
                    getStartPos: () => {
                        let pos = 1
                        for (let i = 0; i < index; i++) {
                            pos +=
                                this.editor.view.state.doc.firstChild.child(
                                    i
                                ).nodeSize
                        }
                        return pos
                ...
                this.sources.push({
                    ...
                    getNodes: () => {
                        ...
                        let fnCount = 0
                        this.editor.view.state.doc.firstChild
                            .child(index)
                            .descendants(node => {
                    ...
                    getStartPos: () => {
                        ...
                        for (let i = 0; i < fnFromIndex; i++) {
                            pos +=
                                this.editor.mod.footnotes.fnEditor.view.state.doc.child(
                                    i
                                ).nodeSize
                        }
                        return pos
        ...
        return promise
            ...
            .then(json => {
                ...
                if (source.text === updatedText) {
                    ...
                    let matches = this.ltFilterMatches(
                        source.badPos,
                        source.matches
                    )
                    matches = this.transMatches(
                        source.getStartPos(),
                        source.posMap,
                        matches
                    )
        ...
        return matches.filter(
            match =>
                !badPos.find(
                    bad =>
                        (match.offset < bad[0] &&
                            match.offset + match.length > bad[0]) ||
                        (match.offset < bad[1] &&
                            match.offset + match.length > bad[1]) ||
                        (match.offset >= bad[0] &&
                            match.offset + match.length <= bad[1])
                )
        )
            ...
            Object.assign(
                {
                    ...
                    to:
                        startPos +
                        this.transPos(match.offset + match.length, posMap, -1)
                },
                match
            )
        )
    ...
    pmFilterMatches(view, matches) {
        ...
        return matches.filter(
            match =>
                view.state.doc.textBetween(match.from, match.to).length ===
                match.to - match.from
        )
    }
